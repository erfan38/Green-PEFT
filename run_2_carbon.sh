...
#SBATCH --job-name=greenpeft2_carbon
#SBATCH --output=greenpeft2_carbon_%j.out
#SBATCH --error=greenpeft2_carbon_%j.err
...
BASE_DIR="${SCRATCH}/greenpeft_runs_carbon"
...
CARBON_INTENSITY=50        # static fallback g CO2/kWh (used if API unavailable)
PUE=1.2
GPU_TDP_W=400
CARBON_ZONE="CA-QC"        # Quebec (Narval cluster)
# ELECTRICITY_MAPS_API_KEY: set as env var before submitting if you have a key
# export ELECTRICITY_MAPS_API_KEY="your_key_here"

echo "=== GREEN MODE (carbon-aware) ==="
python -u src/fine-tune_2.py \
  ...
  --carbon_api_key "${ELECTRICITY_MAPS_API_KEY:-}" \
  --carbon_update_interval 50
...
python -u src/compare_2.py \
  --run_a "${LATEST_GREEN}" \
  --run_b "${LATEST_BASE}" \
  --label_a "Green Carbon-Aware" \
  --label_b "Baseline" \
  --outdir "${OUT_COMPARE}"
