...
compare.py
Usage:
  python compare.py --run_a ./green_peft_test_results/green_YYYYMMDD_HHMMSS \
...
def impact_text(val_a: float, val_b: float) -> str:
    """
    Compares B relative to A: (B - A) / A
    """
    pct = pct_change(val_b, val_a)
    if pct is None:
        return "N/A"
    
    # Invert the percentage so that a reduction (B < A) is mathematically positive
    # which will then naturally format as "+{pct}%". An increase (B > A) will be 
    # mathematically negative and will format with a "-" sign.
    pct = -pct
...
def generate_charts(a: Run, b: Run, outdir: str) -> Tuple[str, str, str, Optional[str]]:
    ...
    # Use disaggregated metrics (prefer PUE-adjusted operational; also show embodied separately in PDF)
    ...
    # Apply hatch patterns (assuming run 'a' is baseline, run 'b' is green)
    r1[0].set_hatch('/')
    if len(r1) > 1:
        r1[1].set_hatch('x')
...
def create_pdf(a: Run, b: Run, fig_energy: str, fig_time: str, fig_co2: str, fig_loss: Optional[str], outdir: str) -> str:
    ensure_dir(outdir)
    ...
    # Prefer PUE-adjusted operational CO2, plus embodied separately
    ...
    content.append(Paragraph("Fine-tuning Comparison Report", title_style))
    ...
    content.append(Paragraph("3. Visualizations", h1))
    ...
    content.append(Paragraph("4. Notes", h1))
    content.append(
        Paragraph(
            "This report is disaggregated: energy, carbon intensity, and PUE-adjustment are reported separately, "
            "and embodied amortization is optional. If real-time power measurement is unavailable, the run report "
            "may indicate a TDP proxy method.",
            body,
        )
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run_a", required=True, help="Path to first run folder (e.g., green_YYYYMMDD_HHMMSS)")
    p.add_argument("--run_b", required=True, help="Path to second run folder (e.g., baseline_YYYYMMDD_HHMMSS)")
...
def main() -> None:
    ...
    print(f"✅ Charts: {fig_energy}, {fig_time}, {fig_co2}" + (f", {fig_loss}" if fig_loss else ""))
    print(f"✅ PDF: {pdf}")
